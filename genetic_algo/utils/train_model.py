...
from cube_model import MLPModel
import torch
import json
import logging
import argparse
...
def main():
    ...
    while True:
        ...
        print(f"Iteration : {index}\tLoss : {loss}")
        ...
        if step_idx % config.lr_decay_interval == 0:
            ...
            print(f"Optimizer new lr : {scheduler.get_last_lr()}")
    ...
    # Save the plot
    plt.savefig(f"models/{name}/tmp/loss.png")
    plt.ioff()
    plt.show()
